...
function variable_voltage_and_phase(m::Model)
    @butcher Dict{Symbol,JuMP.VariableRef}(
        n => @variable(
            m,
            base_name = "voltage[$n]",
            lower_bound = min_voltage(node=n),
            upper_bound = max_voltage(node=n)
        ) for n in node()
    ),
    @butcher Dict{Symbol,JuMP.VariableRef}(n => @variable(m, base_name = "phase[$n]") for n in node())
end
...
function variable_power_generation(m::Model)
    @butcher Dict{Symbol,JuMP.VariableRef}(
        u => @variable(
            m,
            base_name = "real_power_generation[$u]",
            lower_bound = min_real_power_output(unit=u),
            upper_bound = max_real_power_output(unit=u)
        ) for u in unit()
    ),
    @butcher Dict{Symbol,JuMP.VariableRef}(
        u => @variable(
            m,
            base_name = "reactive_power_generation[$u]",
            lower_bound = min_reactive_power_output(unit=u),
            upper_bound = max_reactive_power_output(unit=u)
        ) for u in unit()
    )
end
...
function variable_power_flow_to_node(m::Model)
    @butcher Dict{Tuple,JuMP.VariableRef}(
        (c, n) => @variable(
            m,
            base_name = "real_power_flow_to_node[$c, $n]",
            lower_bound = -long_term_rating(connection=c),
            upper_bound = long_term_rating(connection=c)
        ) for (c, n) in connection__to_node()
    ),
    @butcher Dict{Tuple,JuMP.VariableRef}(
        (c, n) => @variable(
            m,
            base_name = "reactive_power_flow_to_node[$c, $n]",
            lower_bound = -long_term_rating(connection=c),
            upper_bound = long_term_rating(connection=c)
        ) for (c, n) in connection__to_node()
    )
...
function variable_power_flow_from_node(m::Model)
    @butcher Dict{Tuple,JuMP.VariableRef}(
        (c, n) => @variable(
            m,
            base_name = "real_power_flow_from_node[$c, $n]",
            lower_bound = -long_term_rating(connection=c),
            upper_bound = long_term_rating(connection=c)
    ...
    @butcher Dict{Tuple,JuMP.VariableRef}(
        (c, n) => @variable(
            m,
            base_name = "reactive_power_flow_from_node[$c, $n]",
            lower_bound = -long_term_rating(connection=c),
            upper_bound = long_term_rating(connection=c)
        ) for (c, n) in connection__from_node()
    )
...
function objective_min_fuel_cost(m::Model, real_power_generation)
    @objective(m, Min, sum(fuel_cost(unit=u) * real_power_generation[u] for u in unit()))
end
...
function constraint_nodal_balance(
    ...
    real_power_flow,
    real_power_flow_from_node,
    reactive_power_flow_from_node,
    real_power_flow_to_node,
    reactive_power_flow_to_node,
)
    @constraint(
        m,
        real_power_balance[n in node()],
        +sum(real_power_generation[u] for u in unit__node(node=n)) - real_power_demand(node=n) +
        sum(real_power_flow_to_node[c, n] for c in connection__to_node(node=n)) -
        sum(real_power_flow_from_node[c, n] for c in connection__from_node(node=n)) == 0
    )
    @constraint(
        m,
        reactive_power_balance[n in node()],
        +sum(reactive_power_generation[u] for u in unit__node(node=n)) - reactive_power_demand(node=n) +
        sum(reactive_power_flow_to_node[c, n] for c in connection__to_node(node=n)) -
        sum(reactive_power_flow_from_node[c, n] for c in connection__from_node(node=n)) == 0
    )
end
...
function constraint_power_flow(
    ...
    @NLconstraint(
        m,
        [(c, from) in connection__from_node(), to = connection__to_node(connection=c)],
    ...
    @NLconstraint(
        m,
        [(c, to) in connection__to_node(), from = connection__from_node(connection=c)],
