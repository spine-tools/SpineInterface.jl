...
function _entity_class_filtering(ec; _compact::Bool=true, _default::Any=[], kwargs...)
    ...
    # First let's check for potential parameter value filters.
    ...
    # Next, (by)class filters.
    if !isempty(class_kwargs)
        filter!(byent -> _byentity_filter(byent, class_kwargs), byentities)
    end
...
function _get_value(pv::ParameterValue{T}, kw, arg::Entity, upd) where {V,T<:Map{Symbol,V}}
