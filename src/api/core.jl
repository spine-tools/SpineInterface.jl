...
"""
    realize(x, callback=nothing)

Realize the given `Call` and return the result.

If realizing the call involves calling a `PameterValue` with a `TimeSlice` value,
and the optional argument `callback` is not `nothing`, then it will be called without arguments
whenever the `TimeSlice` is `roll!`ed beyond the point where the `ParameterValue` changes.
"""
function realize(x, callback=nothing)
    try
        _do_realize(x, callback)
    catch e
        err_msg = "unable to evaluate expression:\n\t$x\n"
        rethrow(ErrorException("$err_msg$(sprint(showerror, e))"))
    end
end
function realize(call::Call, observer::AbstractObserver)
    callback = () -> _update(observer, realize(call, observer))
    realize(call, callback)
end
